...
import { DanmakuRendererError, InvalidDanmakuConfigurationError, InvalidDanmakuMessageError, } from '../errors.js';
...
export function createDomDanmakuRenderer(options) {
    const maxPoolSize = validatePositiveInteger(options.maxPoolSize, DEFAULT_MAX_POOL_SIZE, 'maxPoolSize');
    const maxBadges = validatePositiveInteger(options.maxBadgesPerMessage, DEFAULT_MAX_BADGES, 'maxBadgesPerMessage');
    const maxTextLength = validatePositiveInteger(options.maxTextLength, DEFAULT_MAX_TEXT_LENGTH, 'maxTextLength');
    const maxMeasurements = validatePositiveInteger(options.maxMeasurementsPerFrame, DEFAULT_MAX_MEASUREMENTS, 'maxMeasurementsPerFrame');
    ...
    function recycleFailedRecord(record) {
        const cleanupSucceeded = destroyDisposables(record.disposables);
        resetRoot(record.root);
        record.root.remove();
    ...
    function populateNode(root, message, disposables) {
        resetElement(root);
        ...
        text.style.color = resolveAccessibleTextColor(message.color, options.backgroundColor, options.fallbackTextColor, minimumContrastRatio);
        root.append(text);
    }
    function renderDefaultIdentity(container, identity) {
    ...
    function notifyAssetError(assetKey) {
        try {
    ...
    function applyPosition(root, item, width) {
...
function destroyDisposables(disposables) {
    let succeeded = true;
...
function validatePositiveInteger(input, fallback, field) {
...
function validateRenderableMessage(message) {
